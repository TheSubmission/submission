...
if __name__ == '__main__':
    ...
    for r in range(num_R_color):
        ...
        res += 'ASSERT(x_2_%s = COPY_IN_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT1_1_%s = ROUNDFUNC_IN_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT2_1_%s = y_2_%s);\n' % (str(r), str(r))

        operation_6bit_l.XOR(r, 1)
        res += 'ASSERT(x_3_%s = XOR_IN1_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(ROUNDFUNC_OUT_1_%s= XOR_IN2_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(y_3_%s = XOR_OUT_1_%s);\n' % (str(r), str(r))
    ...
    for r in range(num_R_color, num_R_linear+num_R_color):
        ...
        res += 'ASSERT(ylinear_2_%s = COPY_IN_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT1_1_%s = ROUNDFUNC_IN_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT2_1_%s = xlinear_2_%s);\n' % (str(r), str(r))

        operation_6bit_l.XOR_linear(r, 1)
        res += 'ASSERT(ylinear_3_%s = XOR_IN1_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(ROUNDFUNC_OUT_1_%s = XOR_IN2_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(xlinear_3_%s = XOR_OUT_1_%s);\n' % (str(r), str(r))
