...
if __name__ == '__main__':
    ...
    for r in range(num_R_color):
        ...
        res += 'ASSERT(x_1_%s = COPY_IN_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT1_1_%s = XOR_IN1_0_%s);\n' % (str(r), str(r))
        res += 'ASSERT(ROUNDFUNC_OUT_0_%s= XOR_IN2_0_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT2_1_%s = ROUNDFUNC_IN_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(y_1_%s = XOR_OUT_0_%s);\n' % (str(r), str(r))

        operation_6bit_l.XOR(r, 1)
        operation_6bit_l.ROUNDFUNC(r, 2)
        operation_6bit_l.COPY(r, 2)
        res += 'ASSERT(x_2_%s = COPY_IN_2_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT1_2_%s = XOR_IN1_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(ROUNDFUNC_OUT_1_%s= XOR_IN2_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT2_2_%s = ROUNDFUNC_IN_2_%s);\n' % (str(r), str(r))
        res += 'ASSERT(y_2_%s = XOR_OUT_1_%s);\n' % (str(r), str(r))

        operation_6bit_l.XOR(r, 2)
        res += 'ASSERT(x_3_%s = XOR_IN1_2_%s);\n' % (str(r), str(r))
        res += 'ASSERT(ROUNDFUNC_OUT_2_%s= XOR_IN2_2_%s);\n' % (str(r), str(r))
        res += 'ASSERT(y_3_%s = XOR_OUT_2_%s);\n' % (str(r), str(r))
    ...
    for r in range(num_R_color, num_R_linear+num_R_color):
        ...
        res += 'ASSERT(COPY_OUT1_0_%s = ROUNDFUNC_IN_0_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT2_0_%s = xlinear_0_%s);\n' % (str(r), str(r))
        ...
        res += 'ASSERT(ROUNDFUNC_OUT_0_%s= XOR_IN2_0_%s);\n' % (str(r), str(r))
        res += 'ASSERT(XOR_OUT_0_%s = COPY_IN_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(xlinear_1_%s = COPY_OUT2_1_%s);\n' % (str(r), str(r))

        operation_6bit_l.XOR_linear(r, 1)
        operation_6bit_l.ROUNDFUNC_linear(r, 2)
        operation_6bit_l.COPY_linear(r, 2)
        res += 'ASSERT(ylinear_2_%s = XOR_IN1_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(COPY_OUT1_2_%s = ROUNDFUNC_IN_2_%s);\n' % (str(r), str(r))
        res += 'ASSERT(ROUNDFUNC_OUT_1_%s= XOR_IN2_1_%s);\n' % (str(r), str(r))
        res += 'ASSERT(XOR_OUT_1_%s = COPY_IN_2_%s);\n' % (str(r), str(r))
        res += 'ASSERT(xlinear_2_%s = COPY_OUT2_2_%s);\n' % (str(r), str(r))

        operation_6bit_l.XOR_linear(r, 2)
        res += 'ASSERT(ylinear_3_%s = XOR_IN1_2_%s);\n' % (str(r), str(r))
        res += 'ASSERT(ROUNDFUNC_OUT_2_%s= XOR_IN2_2_%s);\n' % (str(r), str(r))
        res += 'ASSERT(xlinear_3_%s = XOR_OUT_2_%s);\n' % (str(r), str(r))
