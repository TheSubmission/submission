import itertools
import random
import time
from tqdm import tqdm


S = [
    ,]

#K = [0x37, 0x07, 0x68, 0x54, 0xC1, 0x5E, 0x58, 0x2D, 0x2B, 0xBB, 0xBD, 0xC8, 0xBC, 0x72, 0x65, 0x54, 0xAE, 0x0F, 0xEA, 0x2F, 0xF9]

#C = [0x8D, 0xB1, 0xDE, 0xE2, 0x77, 0x4B, 0x92, 0xAE, 0x3B, 0x07, 0x68, 0x54, 0xC1, 0xEB, 0xD1, 0x2F, 0xDA, 0x12, 0xBB, 0x4B, 0xF8]


def F(input, round,K,C):
    output = S[input^K[round]]^C[round]
    return output

def ROUND(in0,in1,in2,in3,round,K,C):
    out0 = in1 ^ F(in0, round,K,C)
    out1 = in2
    out2 = in3
    out3 = in0
    return out0,out1,out2,out3

def one_a(x,a,c0,c1,order):
    numbers = [0,1,2,3]
    res = [-1] *4
    combinations = list(itertools.permutations(numbers, 2))
    res[combinations[order][0]] = x
    res[combinations[order][1]] = a
    complement = list(set(numbers) - set(combinations[order]))
    res[complement[0]] = c0
    res[complement[1]] = c1

    return res
    


if __name__ == '__main__':

    #ax = 0x4a
    #ay = 0x81
    X0,X1,X2,X3 = [-1]*256,[-1]*256,[-1]*256,[-1]*256
    Y0,Y1,Y2,Y3 = [-1]*256,[-1]*256,[-1]*256,[-1]*256
    f0,f1,f2,f3 = [-1]*256,[-1]*256,[-1]*256,[-1]*256

    c0 = 0x77
    c1 = 0xa1

    R = 9

    K = [random.randint(1, 15) for i in range(20)]
    C = [random.randint(1, 15) for i in range(20)]

    r1 = range(1)
    r2 = range(1)
    Arange = list(itertools.product(r1,r2))
    print(len(Arange))
    for ax,ay in tqdm(Arange):
        ax,ay = [random.randint(0,255) for i in range(2)]
        c0,c1 = [random.randint(0,255) for i in range(2)]
        if ax == ay:
            continue

        for order in range(1):
            for xy in range(256):
                #x0,x1,x2,x3 = one_a(xy,ax,c0,c1,order)
                #y0,y1,y2,y3 = one_a(xy,ay,c0,c1,order)
                x0, x1, x2, x3 = c0,c1,ax,xy
                y0, y1, y2, y3 = c0,c1,ay,xy
                for r in range(R):
                    x0,x1,x2,x3 = ROUND(x0,x1,x2,x3,r,K,C)
                    y0,y1,y2,y3 = ROUND(y0,y1,y2,y3,r,K,C)
                    #print(x0, x1, x2, x3)
                    #print(y0, y1, y2, y3)
                X0[xy],X1[xy],X2[xy],X3[xy] = x0,x1,x2,x3
                Y0[xy],Y1[xy],Y2[xy],Y3[xy] = y0,y1,y2,y3

            for i in range(256):
                f0[i],f1[i],f2[i],f3[i] = X0[i]^Y0[i],X1[i]^Y1[i],X2[i]^Y2[i],X3[i]^Y3[i]

            #print(f0)

            for s in range(1,256):
                counter = [0,0,0,0]
                for x in range(256):
                    if f0[x] == f0[x^s]:
                        counter[0] += 1
                    if f1[x] == f1[x^s]:
                        counter[1] += 1
                    if f2[x] == f2[x^s]:
                        counter[2] += 1
                    if f3[x] == f3[x^s]:
                        counter[3] += 1


                for i in range(4):
                    if counter[i] == 256:
                        print('period: %s, branch: %s' % (str(s), str(i)))
                        print('input0 is : ', c0, c1, ax, 'x')
                        print('input0 is : ', c0, c1, ay, 'x')
                        print('keys are: ', K)
                        print('Round constants are', C)

    













    
    
    
    
